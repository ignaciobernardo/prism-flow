...
export const appTransferMode: ToolcraftTransferMode = {
  animationIntent: {
    loopDuration: {
      evidence:
        "The supplied GIF contains 37 frames at 10 fps, establishing a 3.7 second reference motion period. Glyph Flow maps that period onto a circular noise orbit so the product loop is seamless and forward-only.",
      seconds: 3.7,
      source: "reference",
    },
    mode: "timeline-playback",
  ...
  videoReferenceStudy: {
    acceptanceMapping: [
      {
        acceptanceId: "glyph.content",
        behavior: "A stable, aligned micro-type texture fills the canvas while the reveal changes.",
        frameIds: ["f00", "f08", "f16", "f24", "f32", "f36"],
      },
      {
        acceptanceId: "field.scale",
        behavior: "Large organic type masses and voids change topology instead of moving rigidly.",
        frameIds: ["f00", "f08", "f16", "f24", "f32"],
      },
      {
        acceptanceId: "motion.grain",
        behavior: "Sparse dust and intermediate fragments remain visible near moving boundaries.",
        frameIds: ["f08", "f16", "f24", "f32"],
      },
      {
        acceptanceId: "timeline.playback",
        behavior: "The organic field advances continuously through the observed 3.7 second motion period.",
        frameIds: ["f00", "f08", "f16", "f24", "f32", "f36"],
      },
    ],
    behaviorDecomposition:
      "The reference separates into a stable full-canvas micro-type texture, a low-frequency domain-warped threshold mask, partial-glyph antialiasing at the boundary, sparse dust in voids, and forward temporal evolution. Glyph Flow preserves these behaviors while intentionally improving the source GIF's visible wrap with a circular periodic field.",
    extractionEvidence:
      "ffprobe measured 500x580, 37 frames, 10 fps, and 3.7 seconds. ffmpeg extracted eleven evenly sampled frames at 3 fps into /tmp/glyph-flow-reference and an original-detail contact sheet was inspected before implementation.",
    referenceLocation: "/Users/natochi/Downloads/effect.gif",
    storyboard: [
      {
        behaviorObservation:
          "The type rows remain fixed while the thresholded organic silhouette owns the visible motion.",
        frameId: "f00",
        frameSource: "/Users/natochi/Downloads/effect.gif#frame=0",
        timeSeconds: 0,
        visualObservation:
          "A dense upper band and lower central island are separated by a broad dark void.",
      },
      {
        behaviorObservation:
          "Negative space expands non-linearly and exposes sparse fragments along the edge.",
        frameId: "f08",
        frameSource: "/Users/natochi/Downloads/effect.gif#frame=8",
        timeSeconds: 0.8,
        visualObservation:
          "A large upper-right void sits above a widening lower shelf of micro-type.",
      },
      {
        behaviorObservation:
          "Field coverage rises without changing glyph size, row pitch, or baseline alignment.",
        frameId: "f16",
        frameSource: "/Users/natochi/Downloads/effect.gif#frame=16",
        timeSeconds: 1.6,
        visualObservation:
          "Type fills most of the frame around one large rounded cavity.",
      },
      {
        behaviorObservation:
          "Coarse mass motion and fine edge erosion evolve together across disconnected islands.",
        frameId: "f24",
        frameSource: "/Users/natochi/Downloads/effect.gif#frame=24",
        timeSeconds: 2.4,
        visualObservation:
          "A tall left mass and smaller right island are divided by a vertical channel.",
      },
      {
        behaviorObservation:
          "The reveal continues in one direction without mirrored or ping-pong movement.",
        frameId: "f32",
        frameSource: "/Users/natochi/Downloads/effect.gif#frame=32",
        timeSeconds: 3.2,
        visualObservation:
          "Dense type occupies the upper-left while a rounded right void opens.",
      },
      {
        behaviorObservation:
          "The source approaches its final state with a visible discontinuity back to frame zero.",
        frameId: "f36",
        frameSource: "/Users/natochi/Downloads/effect.gif#frame=36",
        timeSeconds: 3.6,
        visualObservation:
          "A central dark channel separates two large type masses before the source wraps.",
      },
    ],
    transitionAnalysis: [
      {
        behaviorDelta:
          "From f00 to f08 the upper region erodes from right to left while the lower island widens; no rigid object translation explains both changes.",
        fromFrameId: "f00",
        id: "f00-f08",
        toFrameId: "f08",
      },
      {
        behaviorDelta:
          "From f08 to f16 disconnected islands merge into a high-coverage sheet while text baselines stay fixed, proving the mask owns the motion.",
        fromFrameId: "f08",
        id: "f08-f16",
        toFrameId: "f16",
      },
      {
        behaviorDelta:
          "From f16 to f24 one cavity splits into several lobes and a vertical channel, with granular fragments trailing the threshold edge.",
        fromFrameId: "f16",
        id: "f16-f24",
        toFrameId: "f24",
      },
      {
        behaviorDelta:
          "From f24 to f32 coverage expands at the top and left while the right-side void persists, showing coupled coarse and fine field frequencies.",
        fromFrameId: "f24",
        id: "f24-f32",
        toFrameId: "f32",
      },
      {
        behaviorDelta:
          "From f32 to f36 and the wrapped f00 state, the source jumps visibly; Glyph Flow intentionally maps the same forward behavior onto a seamless circular orbit.",
        fromFrameId: "f32",
        id: "f32-f36",
        toFrameId: "f36",
      },
...
export const appProductReadiness: ToolcraftProductReadiness = {
  mode: "product",
  productName: "Glyph Flow",
  productSummary:
    "A procedural typographic motion generator that reveals a stable micro-glyph field through a looping organic noise mask with granular edge dust.",
  requestedBehavior:
    "Create editable procedural motion like the supplied 500x580 GIF: fixed rows of tiny glyphs form large moving organic masses and voids, with controls for the glyph corpus, density, field shape, motion, grain, seed, colors, canvas size, timeline, and PNG/video export.",
};

function glyphFlowControlAcceptance({
  componentType,
  expectedObservable,
  id,
  label,
  ...
  const interaction = componentType === "slider" ? "Drag" : "Change";

  return {
    automated: true,
    automatedTestName: `${id} changes glyph flow product output`,
    browser: true,
    browserTestName: `browser: ${id} changes glyph flow product output`,
    componentType,
    evidence: "product-output",
    expectedObservable,
    fixture: "Glyph Flow default 500x580 field at a fixed timeline frame",
    id,
    kind: "control",
    optionCoverage,
    target: id,
    userAction:
      componentType === "slider"
        ? `${interaction} the ${label} slider and confirm rendered pixels change live while the pointer is still dragging.`
        : `${interaction} ${label} through the visible control and compare rendered product pixels.`,
  };
}

const glyphFlowControlAcceptanceRows: readonly ToolcraftComponentAcceptance[] = [
  glyphFlowControlAcceptance({
    componentType: "text",
    expectedObservable:
      "Typing a different Corpus rebuilds the cached micro-type texture and changes the repeated glyph pixels across the product canvas.",
    id: "glyph.content",
    label: "Corpus",
  }),
  glyphFlowControlAcceptance({
    componentType: "slider",
    expectedObservable:
      "Dragging Size changes the visible micro-glyph height and text density during the drag.",
    id: "glyph.size",
    label: "Size",
  }),
  glyphFlowControlAcceptance({
    componentType: "slider",
    expectedObservable:
      "Dragging Row spacing changes the pitch between stable text rows during the drag.",
    id: "glyph.spacing",
    label: "Row spacing",
  }),
  glyphFlowControlAcceptance({
    componentType: "slider",
    expectedObservable:
      "Dragging Scale changes the size and topology of the organic masses and voids during the drag.",
    id: "field.scale",
    label: "Scale",
  }),
  glyphFlowControlAcceptance({
    componentType: "slider",
    expectedObservable:
      "Dragging Coverage changes the balance of filled type regions and negative space during the drag.",
    id: "field.coverage",
    label: "Coverage",
  }),
  glyphFlowControlAcceptance({
    componentType: "slider",
    expectedObservable:
      "Dragging Warp bends the threshold field into different lobes and channels during the drag.",
    id: "field.warp",
    label: "Warp",
  }),
  glyphFlowControlAcceptance({
    componentType: "slider",
    expectedObservable:
      "Dragging Detail adds or removes fine field octaves and visibly changes boundary erosion during the drag.",
    id: "field.detail",
    label: "Detail",
  }),
  glyphFlowControlAcceptance({
    componentType: "slider",
    expectedObservable:
      "Dragging Edge softness changes partial-glyph antialiasing along the threshold boundary during the drag.",
    id: "field.softness",
    label: "Edge softness",
  }),
  glyphFlowControlAcceptance({
    componentType: "slider",
    expectedObservable:
      "Dragging Cycles changes the field phase at the same timeline position while preserving a forward seamless loop.",
    id: "motion.cycles",
    label: "Cycles",
  }),
  glyphFlowControlAcceptance({
    componentType: "slider",
    expectedObservable:
      "Dragging Drift changes the travel distance of organic field features during the drag.",
    id: "motion.drift",
    label: "Drift",
  }),
  glyphFlowControlAcceptance({
    componentType: "slider",
    expectedObservable:
      "Dragging Grain changes sparse dust and granular fragments around organic boundaries during the drag.",
    id: "motion.grain",
    label: "Grain",
  }),
  glyphFlowControlAcceptance({
    componentType: "text",
    expectedObservable:
      "Committing a different Seed deterministically changes the field, dust, and alternating row offset.",
    id: "motion.seed",
    label: "Seed",
  }),
  glyphFlowControlAcceptance({
    componentType: "color",
    expectedObservable: "Changing Foreground changes the light micro-type and dust color.",
    id: "appearance.foreground",
    label: "Foreground",
  }),
  {
    automated: true,
    automatedTestName:
      "export.includeBackground hides preview background and makes transparent PNG output",
    browser: true,
    browserTestName:
      "browser: export.includeBackground hides preview background and makes transparent PNG output",
    componentType: "switch",
    evidence: "product-output",
    expectedObservable:
      "Turning Include off hides the product-rendered preview background and creates transparent PNG pixels; turning it on restores the selected background, while video always keeps the background.",
    fixture: "Glyph Flow default field with alpha-sensitive background pixels",
    id: "export.includeBackground",
    kind: "control",
    target: "export.includeBackground",
    userAction:
      "Toggle Include off and compare preview/transparent PNG pixels, then toggle it on and confirm PNG background returns and video keeps the background.",
  },
  glyphFlowControlAcceptance({
    componentType: "color",
    expectedObservable:
      "Changing the unlabeled Background color changes preview and included PNG/video background pixels.",
    id: "appearance.background",
    label: "Background",
  }),
  {
    automated: true,
    automatedTestName: "export.image.format changes glyph flow image bytes",
    browser: true,
    browserTestName: "browser: export.image.format changes glyph flow image bytes",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "Selecting PNG and JPG produces decoded image exports with matching MIME types and alpha behavior.",
    fixture: "Image Export Format set to PNG and JPG",
    id: "export.image.format",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "export.image.format",
    userAction: "Select PNG and export, then select JPG and export again.",
  },
  {
    automated: true,
    automatedTestName: "export.image.resolution changes glyph flow image dimensions",
    browser: true,
    browserTestName: "browser: export.image.resolution changes glyph flow image dimensions",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "Selecting 2K, 4K, and 8K produces decoded image exports with 2048, 4096, and 8192 pixel long edges.",
    fixture: "Image Export Resolution set to 2K, 4K, and 8K",
    id: "export.image.resolution",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "export.image.resolution",
    userAction: "Select 2K and export, then select 4K and 8K and compare decoded dimensions.",
  },
  {
    automated: true,
    automatedTestName: "export.video.format changes glyph flow video container",
    browser: true,
    browserTestName: "browser: export.video.format changes glyph flow video container",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "Selecting MP4 and WebM requests the matching supported MediaRecorder container; unsupported MP4 safely falls back to a supported WebM MIME type with valid bytes.",
    fixture: "Video Export Format set to MP4 and WebM",
    id: "export.video.format",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "export.video.format",
    userAction: "Select MP4 and export, then select WebM and export, checking MIME and bytes.",
  },
  {
    automated: true,
    automatedTestName: "export.video.resolution changes glyph flow video dimensions",
    browser: true,
    browserTestName: "browser: export.video.resolution changes glyph flow video dimensions",
    componentType: "select",
    evidence: "exported-bytes",
    expectedObservable:
      "Selecting Current uses even current canvas dimensions and selecting 4K fits inside 3840x2160 with even dimensions and preserved aspect ratio.",
    fixture: "Video Export Resolution set to Current and 4K",
    id: "export.video.resolution",
    kind: "control",
    optionCoverage: "each-visible-item",
    target: "export.video.resolution",
    userAction: "Select Current and export, then select 4K and compare decoded video dimensions.",
  },
  {
    actionCoverage: ["export.video", "export.png"],
    automated: true,
    automatedTestName: "panel.actions export glyph flow video and PNG with progress",
    browser: true,
    browserTestName: "browser: panel.actions export glyph flow video and PNG with progress",
    componentType: "panelActions",
    evidence: "exported-bytes",
    expectedObservable:
      "Export Video returns valid timeline-duration video bytes with frame progress and Export PNG returns decoded image bytes; the sticky accent appears while each Promise is pending and hides after settlement.",
    fixture: "Glyph Flow default field with a short edited timeline duration",
    id: "panel.actions",
    kind: "control",
    target: "panel.actions",
    userAction:
      "Click Export Video and observe frame-based progress/pending state, then click Export PNG and observe pending state until the download starts.",
  },
  {
    automated: true,
    automatedTestName: "canvas.renderScale changes glyph flow backing pixels",
    browser: true,
    browserTestName: "browser: canvas.renderScale changes glyph flow backing pixels",
    componentType: "slider",
    evidence: "viewport-side-effect",
    expectedObservable:
      "Resolution scale 1 and 2 change the real canvas backing dimensions while preserving the 500x580 CSS viewport and product aspect ratio.",
    fixture: "Glyph Flow 500x580 canvas at Resolution scale 1 and 2",
    id: "canvas.renderScale",
    kind: "control",
    target: "canvas.renderScale",
    userAction:
      "Drag the discrete Resolution scale control from 1 to 2 and compare visible canvas backing pixels.",
  },
  {
    automated: true,
    automatedTestName: "connects timeline playback controls to glyph flow renderer",
    browser: true,
    browserTestName: "browser: timeline playback controls glyph flow renderer",
    componentType: "timeline",
    evidence: "timeline-output",
    expectedObservable:
      "Timeline play/pause and scrub change rendered frames; after editing timeline duration, motion advances forward-only in one direction, the first and last frames stitch with no visible jump, direction does not reverse, and no mirror, no yoyo, and no ping-pong behavior appears.",
    fixture: "Glyph Flow 3.7 second playback timeline at start, midpoint, and end minus epsilon",
    ...
    userAction:
      "Play and pause, scrub the timeline, edit timeline duration, then compare first and last frames and the wrapped first frame to confirm they stitch with no visible jump while motion advances forward-only in one direction with no mirror, no yoyo, no ping-pong, and direction does not reverse.",
  },
  {
    automated: true,
    automatedTestName: "glyph flow persistence restores runtime values after reload",
    browser: true,
    browserTestName: "browser: glyph flow persistence restores runtime values after reload",
    componentType: "persistence",
    evidence: "persistence-state",
    expectedObservable:
      "A changed Seed and its rendered product output are restored after a real runtime persistence reload.",
    fixture: "Seed changed from 2703 to 4815 before reload",
    id: "runtime.persistence",
    kind: "runtime",
    persistenceCoverage: "reload",
    target: "runtime.persistence",
    userAction: "Change Seed, wait for localStorage persistence, reload the page, and verify the value/output returns.",
  },
];

export const appAcceptance: readonly ToolcraftComponentAcceptance[] =
  glyphFlowControlAcceptanceRows;

export const starterControlSectionInventory: readonly ToolcraftControlSectionInventoryEntry[] = [
  {
    entity: "Stable micro-type texture",
    groupingReason:
      "Corpus, Size, and Row spacing jointly define the repeating typographic texture before the organic mask is applied.",
    targets: ["glyph.content", "glyph.size", "glyph.spacing"],
    title: "Glyph Field",
  },
  {
    entity: "Organic reveal field",
    groupingReason:
      "Scale, Coverage, Warp, Detail, and Edge softness shape the same thresholded scalar field.",
    targets: ["field.scale", "field.coverage", "field.warp", "field.softness", "field.detail"],
    title: "Organic Field",
  },
  {
    entity: "Periodic field evolution",
    groupingReason:
      "Cycles, Drift, Grain, and Seed alter the deterministic motion and edge texture without duplicating timeline transport.",
    targets: ["motion.seed", "motion.cycles", "motion.drift", "motion.grain"],
    title: "Motion",
  },
  {
    entity: "Typographic ink",
    groupingReason: "Foreground is the single editable ink color used by glyphs and dust.",
    targets: ["appearance.foreground"],
    title: "Ink",
  },
  {
    entity: "Output background",
    groupingReason:
      "The mandatory Background section pairs Include with the unlabeled product background color.",
    targets: ["export.includeBackground", "appearance.background"],
    title: "Background",
  },
  {
    workflowStage: "Still image delivery",
    groupingReason: "Format and Resolution jointly configure the still image output.",
    targets: ["export.image.format", "export.image.resolution"],
    title: "Image Export",
  },
  {
    workflowStage: "Animated delivery",
    groupingReason: "Format and Resolution jointly configure the timeline video output.",
...
export function validateToolcraftAcceptanceCoverage(
  ...
  const resolvedTransferMode: ToolcraftTransferMode =
    transferMode ??
    (schema === appSchema
      ? appTransferMode
      : { animationIntent: { mode: "none" }, mode: "new-toolcraft-app" });
  const resolvedSectionInventory =
    sectionInventory ?? (schema === appSchema ? starterControlSectionInventory : []);
  const errors: string[] = [];
  ...
  const animationIntent = resolvedTransferMode.animationIntent;
  ...
  errors.push(...getToolcraftControlSectionGroupingErrors(schema, resolvedSectionInventory));
  errors.push(...getToolcraftControlSectionInventoryErrors(schema, resolvedSectionInventory));

  const transferModeEvidenceText = getToolcraftTransferModeEvidenceText(resolvedTransferMode);

  if (
    videoReferenceEvidencePattern.test(transferModeEvidenceText) &&
    !resolvedTransferMode.videoReferenceStudy
  ) {
    errors.push(
  ...
  if (resolvedTransferMode.videoReferenceStudy) {
    errors.push(
  ...
  if (
    resolvedTransferMode.mode === "new-toolcraft-app" &&
  ...
  if (resolvedTransferMode.mode === "reference-runtime-clone") {
    const declaredReferenceCoverage = new Set(resolvedTransferMode.behaviorCoverage);
    const referenceTimeline = resolvedTransferMode.referenceTimeline;
    const referenceStudy = resolvedTransferMode.referenceStudy;
    const referenceFeatureInventory = resolvedTransferMode.referenceFeatureInventory ?? [];
    const acceptanceById = new Map(acceptance.map((entry) => [entry.id, entry]));
    ...
    if (!resolvedTransferMode.referenceName.trim()) {
      errors.push(
        "reference-runtime-clone transferMode must name the reference app or artifact.",
      );
    }

    if (resolvedTransferMode.sourceOfTruth !== "reference-runtime") {
      errors.push(
